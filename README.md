...
每次通过 Bridge 新建 Codex 对话（包括 `/new` 后的第一条消息、启动卡和 `/fork`），都会启动新的 app-server，由 Codex 重新读取当前登录凭据。因此在本机重新登录、切换账号或退出后，新对话会使用最新登录状态。已有聊天继续使用各自的进程；被新会话替换的旧进程会在任务结束、且没有活动目标时释放。
...
| `/model` | `/model` | 打开下拉框选择当前聊天/话题的 Codex 模型与 reasoning effort，应用后另发消息确认当前设置。 |
| `/fast` | `/fast` | 查看或切换快速模式。 |
