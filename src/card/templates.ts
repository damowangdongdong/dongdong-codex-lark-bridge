...
export function helpCard(agentName = 'Agent', botName?: string): object {
  ...
  const panels: object[] = [
    ...
    tutorialPanel('🧾 Bridge 命令速查', [
      ...
      '- `/interupt` → 示例：`/interupt`；效果：等效 Codex Esc，打断当前 turn并保留排队内容；无活动任务但有队列时立即执行队列。',
      '- `/interrupt` → 示例：`/interrupt`；效果：`/interupt` 的正确拼写别名。',
      '- `/stop` → 示例：`/stop`；效果：立即停止当前任务并清空排队内容；Codex 没有活动 turn 时停止本 thread 的后台终端。',
    ...
    tutorialPanel('🧩 Codex 命令与执行位置', [
      ...
      '- `/queue <指令>` → 示例：`/queue 检查测试`；效果：把指令排到当前 turn 的下一轮。',
      '- `/interupt` → 示例：`/interupt`；效果：等效 Codex Esc，打断活动 turn 并保留队列；没有活动 turn 时立即执行已有队列。',
      '- `/interrupt` → 示例：`/interrupt`；效果：`/interupt` 的正确拼写别名。',
      '- `/stop` → 示例：`/stop`；效果：立即停止活动 turn 并清空排队消息；没有活动 turn 时停止后台终端。',
